...
const SESSION_TIMEOUT_ADMIN_MS = 1 * 60 * 1000; // 1 hour (admin)
const SESSION_TIMEOUT_KEPALA_MS = 1 * 60 * 60 * 1000; // 1 hour (kepala)
const SESSION_TIMEOUT_EMPLOYEE_MS = 1 * 60 * 1000; // 1 minute (employee)

// Session auto-clears when browser/app is closed (sessionStorage).
// Additionally, session expires based on role (admin/employee: 1 min, kepala: 1 hour).
export default function Page() {
  ...
  useEffect(() => {
    ...
    const getTimeout = () => {
      if (user.role === "kepala") return SESSION_TIMEOUT_KEPALA_MS;
      return SESSION_TIMEOUT_ADMIN_MS; // admin & employee: 1 minute
    };
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed && parsed.id && parsed.role) {
          // Check if session has already expired based on role
          const sessionTime = sessionStorage.getItem(SESSION_TIME_KEY);
          if (sessionTime) {
            const elapsed = Date.now() - parseInt(sessionTime, 10);
            const timeout =
              parsed.role === "kepala"
                ? SESSION_TIMEOUT_KEPALA_MS
                : SESSION_TIMEOUT_ADMIN_MS;
            if (elapsed < timeout) {
              restoredUser = parsed;
            }
          }
